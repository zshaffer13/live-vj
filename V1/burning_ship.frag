/* burning_ship.frag (Double Precision, Shadows, Audio Warp, Color Schemes) */
#version 430 core
...
#include "color.glsl"  // Import getColor and hsv2rgb()

void main() {
    // 1. Aspect-correct UV space
    dvec2 uv = dvec2(fragCoord);
    uv.x = (uv.x - 0.5) * (resolution.x / resolution.y) + 0.5;

    // 2. Audio-driven warp distortion
    ...
    // 3. Map to complex space
    dvec2 c = dvec2(
        (uv.x - 0.5) * zoom + offset.x,
        (uv.y - 0.5) * zoom + offset.y
    );

    // 4. Rotate c around center
    dvec2 delta = c - offset;
    double cR = dcos(rotation), sR = dsin(rotation);
    c = dvec2(delta.x * cR - delta.y * sR,
              delta.x * sR + delta.y * cR) + offset;

    // 5. Burning ship iteration (abs applied after squaring)
    dvec2 z = dvec2(0.0);
    int i;
    ...
    for (i = 0; i < max_iterations; i++) {
        ...
        z = dvec2(z.x * z.x - z.y * z.y, 2.0 * z.x * z.y);
        z = dvec2(abs(z.x), abs(z.y)) + c;
    }

    // 6. Color output using scheme + optional shadows
    if (i == max_iterations) {
        fragColor = vec4(0.0);
    } else {
