/* julia.frag (Double Precision, Shadows, Audio Warp, Color Schemes) */
#version 430 core
...
// Float-based helpers for double precision logic
...
#include "color.glsl"  // Centralized color palette logic

void main() {
    // 1. UV space with aspect correction
    ...
    // 3. Map UV to complex coordinate space
    dvec2 c = dvec2((uv.x - 0.5) * zoom + offset.x,
                    (uv.y - 0.5) * zoom + offset.y);

    // 4. Apply rotation around center point
    ...
    // 6. Final color based on scheme and shadowing
    if (i == max_iterations) {
        fragColor = vec4(0.0);
    } else {
        ...
        // Optional shadowing (darker edges)
        if (enable_shadows == 1) {
